    ...
    <aside className="w-56 h-full bg-sidebar flex flex-col shrink-0 border-r border-sidebar-border">
      ...
      <div className="px-2 py-2 border-b border-sidebar-border">
        ...
        >
          ...
          <span className={cn('text-sidebar-text transition-transform', orgOpen && 'rotate-180')}>
            {Icon.chevronDown}
          </span>
        </button>

        {orgOpen && (
          <div className="mt-1 mx-1 bg-zinc-800 rounded border border-zinc-700 overflow-hidden">
            {orgs.map((org) => (
              <button
                key={org.id || org._id}
                onClick={() => {
          ...
          <>
            ...
            {boards.map((board) => (
              <Link
                ...
                className={cn(
                  'nav-item',
                  isActiveBoard(board.id || board._id) && 'active',
                )}
              >
